...
class CMDataset(data.Dataset):
    ...
    def open_data(self):
        ...
        if self.partition == 'train' : data = np.load('/root/dataset/nuswide/train.npz')
        elif self.partition == 'retrieval': data = np.load('/root/dataset/nuswide/database.npz')
        elif self.partition == 'test' : data = np.load('/root/dataset/nuswide/test.npz')
        else : print("cnt"*100)
        self.imgs = data['img'] #.transpose([0, 3, 2, 1])
        self.texts = data['txt']
