...
logging.basicConfig(
    ...
    datefmt="%m/%d/%Y %H:%M:%S",
    level=logging.INFO,
    filename=f"./log/ori_h_cat_cy_{args.num_labeled}.log", 
)
...
if __name__ == '__main__':
    ...
    for args.code_length in [16]:
        ...
        criterion = ContrastiveLoss(.2, .1)
        n_data = len(train_labeled_loader.dataset)
        contrast = NCEAverage(args.code_length, n_data, 4096, .9, .4)
        ...
        for epoch in range(args.epoch_num):
            ...
            Loss_SM = 0
            Loss_CY = 0
            train_loader = train_labeled_loader # zip(cycle(train_labeled_loader), train_us_loader, train_uw_loader)
            for batch_idx, (idx, img, txt, label) in enumerate(train_loader):
                cnt += len(label)
                img = [_img.float().cuda() for _img in img]
                txt = [_txt.float().cuda() for _txt in txt]
                idx = [idx.cuda()]
                label = label.float().cuda()
                out = [image_model(im) for im in img]
                img_code = [_out[0] for _out in out]
                img_pred = [_out[1] for _out in out]
                loss_cy = CLoss(torch.sigmoid(torch.cat(img_pred)), label)
                out = [text_model(tx) for tx in txt]
                txt_code = [_out[0] for _out in out]
                #img_code, img_pred = image_model(img)
                #txt_code, txt_pred = text_model(txt)
                out_l, out_ab = contrast(torch.cat(img_code), torch.cat(txt_code), torch.cat(idx*len(img)), epoch=epoch-1)
                l_loss = criterion_contrast(out_l)
                ab_loss = criterion_contrast(out_ab)
                Lc = l_loss + ab_loss
                Lr = criterion(torch.cat(img_code), torch.cat(txt_code))
                loss = Lc * 0.7 + Lr * 0.3 + loss_cy
                ...
                loss.backward()
                clip_grad_norm_(image_model.parameters(), 1.)
                clip_grad_norm_(text_model.parameters(), 1.)
                image_optimizer.step()
                text_optimizer.step()
            logger.info("[Num]\t: {}".format(cnt))
