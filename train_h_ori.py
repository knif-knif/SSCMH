...
if __name__ == '__main__':
    ...
    for args.code_length in [16]:
       ...
       # train_labeled_loader, _, _ = get_trans_data(args, need_loader=True, return_index=True)
        ...
        print(len(train_dataset), n_data)
        ...
        lr_i = 0.0001
        lr_t = 0.0001
        lr_decay = np.exp(np.linspace(0, -8, args.Epochs))
        ...
        for epoch in range(args.epoch_num):
            ...
            for batch_idx, (idx, img, txt, label) in enumerate(train_loader):
                ...
                out_l, out_ab = contrast(torch.cat(img_code), torch.cat(txt_code), torch.cat(idx*len(img)), epoch=epoch)
            ...
            logger.info("[Loss]\t: Loss : {:.8f}".format(Loss / cnt))
